...
public class KmeanTests {
	...
	private boolean kmeanTester(String jsonString, int numClusters) {
		...
		for (int index : clusters.keySet()) {
			...
			/*
			 * double aveX = list.stream() .mapToDouble(i ->
			 * Double.parseDouble(i.getString("x"))) .average() .getAsDouble();
			 * 
			 * double aveY = list.stream() .mapToDouble(i ->
			 * Double.parseDouble(i.getString("y"))) .average() .getAsDouble();
			 */
			...
			for (JsonObject restaurant : clusters.get(i)) {
				...
				for (int j = 0; j < numClusters && j != i; j++) {
					...
					if (minDist > distToCoordinate(x, y, clusterCenters.get(j))) {
						...
						// System.out.println(minDist + " " + distToCoordinate(x, y,
						// clusterCenters.get(j)));
